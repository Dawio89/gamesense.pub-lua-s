...
local function SwastikaCrosshair()
	pLocal = entity_get_local_player()
	...
	if ui_get(crosshairEnable) then
		...
		local crosshairSize = ui_get(crosshiarSize)*.1
		local a = math_floor(screen.y / 2 / 30) * crosshairSize  -- Adjust the crosshair size here
		...
		for i = 0, 3 do
			...
			if not ui_get(crosshairRainbow) then
				...
				r = math_floor(math_sin(globals_realtime() * 2) * 127 + 128)
				g = math_floor(math_sin(globals_realtime() * 2 + 2) * 127 + 128)
				b = math_floor(math_sin(globals_realtime() * 2 + 4) * 127 + 128)
...
client_set_event_callback("paint",
SwastikaCrosshair
)
